...
export default function ChatInterface() {
  ...
  return (
    <div className="flex-1 flex flex-col w-full overflow-hidden">
      ...
      >
        ...
        {messages.length === 0 ? (
          <div className="flex flex-col items-center justify-center h-full text-center px-4">
            ...
            <h2 className="text-3xl font-bold text-foreground mb-3">Ask me about Rutwik</h2>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 w-full max-w-lg">
              {INITIAL_SUGGESTIONS.map(({ text, icon: Icon }, index) => (
                <button
                  ...
                  className="flex items-center gap-4 p-5 rounded-2xl border border-border bg-card hover:bg-muted/50 hover:border-primary/30 hover:shadow-lg hover:shadow-primary/5 transition-all duration-200 text-left group"
                >
          ...
          <div className="space-y-8">
            {messages.map((msg, index) => (
              <div key={index} className={`flex gap-4 ${msg.role === 'user' ? 'justify-end' : ''}`}>
                {msg.role === 'assistant' && (
                  <div className="w-9 h-9 rounded-xl bg-gradient-to-br from-blue-500/20 to-purple-500/20 flex items-center justify-center flex-shrink-0 mt-1">
                    <Bot className="w-5 h-5 text-blue-500" />
                  </div>
                )}
                <div className={`group relative max-w-[85%] ${msg.role === 'user' ? 'bg-primary text-primary-foreground' : 'bg-card border border-border'} rounded-2xl px-5 py-4`}>
                  {msg.role === 'assistant' ? (
                    <>
                      <div className="prose prose-neutral dark:prose-invert prose-base max-w-none
                        [&>p]:mb-4 [&>p]:leading-relaxed [&>p]:text-foreground
                        [&>ul]:mb-4 [&>ul]:space-y-1 [&>ul]:list-disc [&>ul]:pl-5
                        [&>ol]:mb-4 [&>ol]:space-y-1 [&>ol]:list-decimal [&>ol]:pl-5
                      ...
                      {msg.content && (
                        <button
                          ...
                          className="absolute -bottom-3 right-3 opacity-0 group-hover:opacity-100 transition-opacity p-1.5 rounded-lg bg-muted hover:bg-muted/80 border border-border"
              ...
              <div className="flex flex-wrap gap-2 ml-13">
                {currentSuggestions.map((suggestion, index) => (
                  <button
                    ...
                    className="text-sm px-4 py-2 rounded-xl border border-border bg-card hover:bg-muted hover:border-primary/30 transition-all duration-200 text-muted-foreground hover:text-foreground"
                  >
                    {suggestion}
                  </button>
                ))}
      ...
      {/* Input Area */}
      <div className="border-t border-border bg-background p-4 flex-shrink-0 relative z-10">
        <form onSubmit={handleSubmit} className="max-w-3xl mx-auto">
          <div className="relative flex items-end gap-3 bg-card border border-border rounded-2xl p-3 focus-within:border-primary/50 focus-within:shadow-lg focus-within:shadow-primary/5 transition-all duration-200">
            <textarea
              ref={inputRef}
              placeholder="Ask about Rutwik's experience, skills, or projects..."
              value={input}
              onChange={(e) => setInput(e.target.value)}
              onKeyDown={handleKeyDown}
              disabled={isLoading}
              rows={1}
              className="flex-1 bg-transparent resize-none px-2 py-2 text-base text-foreground placeholder:text-muted-foreground focus:outline-none max-h-[200px] leading-relaxed"
            />
            <button
              type="submit"
              disabled={!input.trim() || isLoading}
              className="p-3 rounded-xl bg-primary text-primary-foreground disabled:opacity-40 disabled:cursor-not-allowed hover:bg-primary/90 transition-colors"
            >
